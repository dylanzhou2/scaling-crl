...
"""
Push-Sphere: identical task to Push-Hard (move an object from a random location on
the blue region to a random goal on the red region), but the object is a *sphere*
instead of a cube. A sphere rolls under contact, so the non-prehensile push is
harder and less forgiving than the box version.
...
The only difference from ArmPushHard is the XML asset (sphere geom) and env_name;
all qpos indexing is unchanged because the sphere keeps the same 7-dim freejoint.

See _get_obs() and ArmEnvs._convert_action() for details.
"""


class ArmPushSphere(ArmEnvs):

  def _get_xml_path(self):
    return "envs/assets/panda_push_sphere.xml"
