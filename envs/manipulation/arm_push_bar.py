...
"""
Push-Bar: identical task to Push-Hard (move an object from a random location on the
blue region to a random goal on the red region), but the object is an elongated
*bar* (a low, long box) instead of a cube. The bar neither rolls nor tips, but it
rotates when pushed off-center, making it a richer non-prehensile push than the
symmetric cube. A box is used (not a cylinder) because MJX does not implement
cylinder collisions.
...
The only difference from ArmPushHard is the XML asset (bar geom) and env_name;
all qpos indexing is unchanged because the bar keeps the same 7-dim freejoint.

See _get_obs() and ArmEnvs._convert_action() for details.
"""


class ArmPushBar(ArmEnvs):

  def _get_xml_path(self):
    return "envs/assets/panda_push_bar.xml"
  ...
  def _get_obs(
    ...
    Note q is 23-dim: 7-dim cube position/angle, 7-dim goal marker
    position/angle, 7-dim joint angles, 2-dim finger offset
