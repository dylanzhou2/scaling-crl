...
class ArmGrasp(ArmEnvs):
  ...
  def _compute_goal_completion(self, obs, goal):
    ...
    cube_to_fingertip_midpoint_dist = jnp.linalg.norm(
        cube_pos - fingertip_midpoint
    )
  ...
  def _get_obs(
    ...
    """Observation space (23-dim)
     ...
     - Fingertips (7-dim): 6-dim positions of fingertips, 1-dim distance between
     fingertips
    Note q is 25-dim: 7-dim cube position/angle, 7-dim goal marker
    position/angle, 7-dim joint angles, 4-dim finger offsets and dummy fingertip
    angles

    Goal space (7-dim): position of left fingertip, position of right fingertip,
