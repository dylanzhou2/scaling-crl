...
class CrlEvaluator:
  ...
  def run_evaluation(
      self, training_state, training_metrics, aggregate_episodes=True
  ):
    ...
    print(
        "Available keys in episode_metrics:",
    ...
    for fn, suffix in aggregating_fns:
      metrics.update({
          ...
          if name
          in eval_metrics.episode_metrics  # THIS WAS ADDED BY ME (for arm tasks, may not be)
      })
